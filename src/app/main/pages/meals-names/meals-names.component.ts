...
@Component({
  selector: 'app-meals-names',
  templateUrl: './meals-names.component.html',
  styleUrls: ['./meals-names.component.scss']
})
export class MealsNamesComponent implements OnInit {
  ...
  getTotalUsersCount() {
    const exercisesCollection = collection(this.firestore, 'meals-names');
    getDocs(exercisesCollection).then(snapshot => {
      this.totalUsersCount = snapshot.size;
    }).catch(error => console.error("Помилка отримання кількості страв: ", error));
  }

  getUsers(pageIndex: number = 0, newPageSize: number = this.pageSize): void {
    this.pageSize = newPageSize; // ✅ Оновлюємо `pageSize`

    let exercisesCollection = collection(this.firestore, 'meals-names');
    let q = query(
      exercisesCollection,
      limit(this.pageSize)
    );
    ...
    }).catch(error => console.error("Помилка отримання страв: ", error));
  }
    ...
    getDocs(q).then(snapshot => {
      if (!snapshot.empty) {
        snapshot.forEach(docSnapshot => {
          const docRef = doc(this.firestore, 'meals-names', docSnapshot.id);
          deleteDoc(docRef).then(() => {
            this.snackBar.open('страву успішно видалено', 'Закрити', { duration: 2000 });
            this.getUsers(); // Оновлюємо список після видалення
          }).catch(error => console.error("Помилка видалення страви:", error));
        });
      }
    }).catch(error => console.error("Помилка пошуку страви:", error));
  }

  moveToExegesis(id) {
    this.router.navigate([`/meals/edit-meals`, id]);
  }
}
